...
function Tag({ label, color, selected, onClick }: {
  ...
  return (
    <button onClick={onClick} style={{
      ...
      background: selected ? `${color}20` : `${color}08`,
...
function ScoreBar({ label, value, color }: { label: string; value: number; color: string }) {
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 12, padding: "9px 14px", background: "var(--surface)", borderRadius: 7, border: "1px solid var(--border)" }}>
      <span style={{ fontFamily: "var(--font-mono)", fontSize: 9, letterSpacing: 2, textTransform: "uppercase" as const, color: "var(--text-dim)", whiteSpace: "nowrap", minWidth: 110 }}>{label}</span>
      <div style={{ flex: 1, height: 3, background: "var(--border)", borderRadius: 2, overflow: "hidden" }}>
        <div style={{ width: `${value}%`, height: "100%", background: color, borderRadius: 2, transition: "width 1.2s ease" }} />
      </div>
      <span style={{ fontFamily: "var(--font-mono)", fontSize: 13, fontWeight: 500, color, minWidth: 40, textAlign: "right" }}>{value}%</span>
    </div>
  );
}
...
export default function Home() {
  ...
  // Filter library entries relevant to current use case
  ...
  // Collect prompt contributions from selected tags
    ...
    <div style={{ minHeight: "100vh", display: "flex", flexDirection: "column" }}>
      ...
      <header style={{
        ...
        <div style={{ display: "flex", alignItems: "baseline", gap: 10 }}>
          ...
          <span style={{ fontSize: 9, fontWeight: 700, letterSpacing: 3, color: "var(--accent)", padding: "3px 8px", border: "1px solid var(--accent)", borderRadius: 2 }}>PRO v4</span>
          ...
          <div style={{ flex: 1, overflowY: "auto", padding: "20px 24px", display: "flex", flexDirection: "column", gap: 20 }}>
            ...
            <div>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 6 }}>
                {USE_CASES.map(uc => (
                  <button key={uc.id} onClick={() => { setUseCase(uc.id); setSelectedTags([]); }} style={{
                    padding: "8px 6px", border: `1px solid ${useCase === uc.id ? "var(--accent)" : "var(--border)"}`,
                    borderRadius: 7, background: useCase === uc.id ? "rgba(255,77,0,0.1)" : "var(--surface)",
                    cursor: "pointer", textAlign: "center", transition: "all 0.15s",
                  }}>
                    <div style={{ fontSize: 14, marginBottom: 3 }}>{uc.icon}</div>
                    <div style={{ fontSize: 10, fontWeight: 700, color: useCase === uc.id ? "var(--accent)" : "var(--text-muted)", fontFamily: "var(--font-display)" }}>
                      {uc.label}
                    </div>
                  </button>
                ))}
            ...
            <div>
              <div style={{ fontSize: 10, letterSpacing: 2.5, textTransform: "uppercase", color: "var(--text-muted)", fontFamily: "var(--font-mono)", marginBottom: 10, display: "flex", alignItems: "center", gap: 8 }}>
                <span style={{ width: 6, height: 6, borderRadius: "50%", background: "var(--accent)", display: "inline-block" }} />
                Zusätzliche Intention <span style={{ color: "var(--text-dim)", fontStyle: "normal" }}>(optional)</span>
              </div>
              <textarea
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
              {[
                {
                  label: "Stil-Tonalität", val: tone, set: setTone,
                  opts: [["luxury", "Luxus / High-End"], ["documentary", "Dokumentarisch / Roh"], ["editorial", "Editorial / Fashion"], ["dark", "Dunkel / Dramatisch"], ["artistic", "Künstlerisch / Abstrakt"], ["commercial", "Kommerziell / Klar"]],
                },
                ...(mode === "video" ? [{
                  label: "Videolänge", val: duration, set: setDuration,
                  opts: [["3", "3 Sek — Micro"], ["5", "5 Sek — Standard"], ["8", "8 Sek — Hero Shot"], ["15", "15 Sek — Feature"]],
                }] : []),
                ...
                <div key={label}>
                  ...
                  <div style={{ position: "relative" }}>
                    <select value={val} onChange={e => (set as (v: string) => void)(e.target.value)} style={{ width: "100%", appearance: "none", background: "var(--surface)", border: "1px solid var(--border2)", borderRadius: 6, color: "#c8c4be", fontSize: 11, padding: "9px 28px 9px 12px", cursor: "pointer" }}>
            ...
            {!output && step === 0 && (
              <div style={{ height: "100%", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 16, textAlign: "center", opacity: 0.3 }}>
                ...
                <div style={{ fontFamily: "var(--font-mono)", fontSize: 11, color: "#555", lineHeight: 2 }}>
                  Verwendungszweck wählen → Bild hochladen<br />
                  Gemini extrahiert alle Details → Claude baut den Prompt<br />
                  Kopieren → In dein AI-Tool einfügen
                </div>
              </div>
            )}
            ...
            {isLoading && !output && (
              <div style={{ height: "100%", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 20 }}>
                ...
                <div style={{ fontFamily: "var(--font-mono)", fontSize: 10, color: "#333", letterSpacing: 1, textAlign: "center", lineHeight: 1.8 }}>
                  {step === 1
                    ? "Extrahiert: Farben · Materialien · Licht · Komposition · Details"
                    : "Fusioniert: Bilddaten + Intention + Orchestra Library → $1M Schema"}
                </div>
              </div>
            )}
            ...
            {output && (
              <div style={{ display: "flex", flexDirection: "column", gap: 16, animation: "fadeUp 0.4s ease" }}>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                  <ScoreBar label="Prompt Qualität" value={qScore} color={qColor} />
                  <ScoreBar label="Detail-Genauigkeit" value={dScore} color={dColor} />
                </div>

                {/* Ghost Director */}
                {output.ghost_director && (
                  <div style={{ padding: "11px 15px", background: "rgba(255,77,0,0.03)", border: "1px solid rgba(255,77,0,0.1)", borderRadius: 8, fontFamily: "var(--font-serif)", fontStyle: "italic", fontSize: 13, color: "#666", lineHeight: 1.7 }}>
                    "{output.ghost_director}"
                  </div>
                )}
                ...
                {activeTab === "layers" && output.layers && (
                  <div>
                    ...
                    <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                      {[
                        ["world", "Welt / Umgebung"],
                        ["subject", "Subjekt / Detail"],
                        ["motion", "Bewegung"],
                        ["lighting", "Licht"],
                        ["lens", "Linse / Optik"],
                        ["color", "Farbgebung"],
                        ["physics", "Physik"],
                        ["intention", "Intention / Gefühl"],
                      ].map(([key, label]) => {
                        const val = output.layers[key as keyof typeof output.layers];
                        if (!val) return null;
                        return (
                          <div key={key} style={{ display: "grid", gridTemplateColumns: "100px 1fr", gap: 10, alignItems: "start" }}>
                            <span style={{ fontFamily: "var(--font-mono)", fontSize: 9, color: "var(--text-muted)", textTransform: "uppercase", letterSpacing: 1.5, paddingTop: 6 }}>{label}</span>
                            <span style={{ fontFamily: "var(--font-mono)", fontSize: 12, color: "#c8c4be", background: "var(--surface)", borderRadius: 5, padding: "6px 10px", border: "1px solid var(--border)", lineHeight: 1.6 }}>{val}</span>
                          </div>
                        );
                      })}
                    </div>
